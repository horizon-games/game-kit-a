...
import { Controller, ControllerEdge } from './Controller'
  ...
  Step(step: TimeStep) {
    ...
    for (let i: ControllerEdge | null = this.m_bodyList; i; i = i.nextBody) {
      ...
      for (
        ...
        areac.x += sarea * sc.x
        areac.y += sarea * sc.y
        const shapeDensity = this.useDensity ? fixture.GetDensity() : 1
