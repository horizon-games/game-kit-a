...
} from './Contacts/ContactSolver'
import { Joint } from './Joints/Joint'
import { Position, Profile, SolverData, TimeStep, Velocity } from './TimeStep'
import { ContactImpulse, ContactListener } from './WorldCallbacks'
...
export class Island {
  ...
  m_positions: Position[] = Position.MakeArray(1024) // TODO: Settings
  m_velocities: Velocity[] = Velocity.MakeArray(1024) // TODO: Settings
  ...
  Solve(
    ...
    for (let i: number = 0; i < this.m_bodyCount; ++i) {
      const b: Body = this.m_bodies[i]

      // const c: Vec2 =
      this.m_positions[i].c.Copy(b.m_sweep.c)
      const a: number = b.m_sweep.a
      const v: Vec2 = this.m_velocities[i].v.Copy(b.m_linearVelocity)
    ...
    const contactSolver: ContactSolver = Island.s_contactSolver.Initialize(
      contactSolverDef
    )
    ...
    // Integrate positions.
    for (let i: number = 0; i < this.m_bodyCount; ++i) {
      const c: Vec2 = this.m_positions[i].c
      let a: number = this.m_positions[i].a
      const v: Vec2 = this.m_velocities[i].v
    ...
    for (let i: number = 0; i < step.positionIterations; ++i) {
      ...
      for (let j: number = 0; j < this.m_jointCount; ++j) {
        const jointOkay: boolean = this.m_joints[j].SolvePositionConstraints(
          solverData
        )
  ...
  SolveTOI(subStep: TimeStep, toiIndexA: number, toiIndexB: number): void {
    ...
    for (let i: number = 0; i < subStep.positionIterations; ++i) {
      const contactsOkay: boolean = contactSolver.SolveTOIPositionConstraints(
        toiIndexA,
        toiIndexB
      )
      if (contactsOkay) {
  ...
  */
  ...
  Report(constraints: ContactVelocityConstraint[]): void {
    ...
    for (let i: number = 0; i < this.m_contactCount; ++i) {
      const c: Contact = this.m_contacts[i]
