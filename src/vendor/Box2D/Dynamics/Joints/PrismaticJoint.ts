...
export class PrismaticJoint extends Joint {
  ...
  private static SolveVelocityConstraints_s_df2 = new Vec2()
  ...
  m_invIA: number = 0
  m_invIB: number = 0
  readonly m_axis: Vec2 = new Vec2(0, 0)
  readonly m_perp: Vec2 = new Vec2(0, 0)
  m_s1: number = 0
  m_s2: number = 0
  m_a1: number = 0
  m_a2: number = 0
  readonly m_K: Mat33 = new Mat33()
  readonly m_K3: Mat33 = new Mat33()
  readonly m_K2: Mat22 = new Mat22()
  ...
  InitVelocityConstraints(data: SolverData): void {
    ...
    if (this.m_enableLimit) {
      ...
      const jointTranslation: number = Vec2.DotVV(this.m_axis, d)
      if (
    ...
    if (data.step.warmStarting) {
      ...
      const P: Vec2 = Vec2.AddVV(
        Vec2.MulSV(this.m_impulse.x, this.m_perp, Vec2.s_t0),
        Vec2.MulSV(
  ...
  SolveVelocityConstraints(data: SolverData): void {
    ...
    if (this.m_enableMotor && this.m_limitState !== LimitState.e_equalLimits) {
      ...
      const Cdot: number =
        Vec2.DotVV(this.m_axis, Vec2.SubVV(vB, vA, Vec2.s_t0)) +
        this.m_a2 * wB -
        this.m_a1 * wA
      ...
      const P: Vec2 = Vec2.MulSV(
        impulse,
        this.m_axis,
        PrismaticJoint.SolveVelocityConstraints_s_P
      )
    ...
    const Cdot1_x: number =
      Vec2.DotVV(this.m_perp, Vec2.SubVV(vB, vA, Vec2.s_t0)) +
      this.m_s2 * wB -
      this.m_s1 * wA
    // Cdot1.y = wB - wA;
    const Cdot1_y = wB - wA
    ...
    if (
      ...
      // Vec3 f1 = m_impulse;
      const f1 = PrismaticJoint.SolveVelocityConstraints_s_f1.Copy(
        this.m_impulse
      )
      ...
      const df3 = this.m_K.Solve33(
        ...
        -Cdot1_y,
        -Cdot2,
        PrismaticJoint.SolveVelocityConstraints_s_df3
      )
      // m_impulse += df;
      ...
      const f2r = this.m_K.Solve22(
        b_x,
        b_y,
        PrismaticJoint.SolveVelocityConstraints_s_f2r
      )
      f2r.x += f1.x
      f2r.y += f1.y
      // m_impulse.x = f2r.x;
      ...
      const P: Vec2 = Vec2.AddVV(
        Vec2.MulSV(df3.x, this.m_perp, Vec2.s_t0),
        Vec2.MulSV(df3.z, this.m_axis, Vec2.s_t1),
        PrismaticJoint.SolveVelocityConstraints_s_P
      )
      ...
      const df2 = this.m_K.Solve22(
        -Cdot1_x,
        -Cdot1_y,
        PrismaticJoint.SolveVelocityConstraints_s_df2
      )
  ...
  SolvePositionConstraints(data: SolverData): boolean {
    ...
    const axis: Vec2 = Rot.MulRV(qA, this.m_localXAxisA, this.m_axis)
    ...
    const perp: Vec2 = Rot.MulRV(qA, this.m_localYAxisA, this.m_perp)
    ...
    if (active) {
      ...
      const impulse1 = K2.Solve(
        -C1_x,
        -C1_y,
        PrismaticJoint.SolvePositionConstraints_s_impulse1
      )
  ...
  GetJointTranslation(): number {
    ...
    const axis = this.m_bodyA.GetWorldVector(
      this.m_localXAxisA,
      PrismaticJoint.GetJointTranslation_s_axis
    )
  ...
  GetJointSpeed(): number {
    ...
    const rA: Vec2 = Rot.MulRV(bA.m_xf.q, this.m_lalcA, this.m_rA)
    ...
    const rB: Vec2 = Rot.MulRV(bB.m_xf.q, this.m_lalcB, this.m_rB)
    // Vec2 pA = bA->m_sweep.c + rA;
    const pA: Vec2 = Vec2.AddVV(bA.m_sweep.c, rA, Vec2.s_t0) // pA uses s_t0
    // Vec2 pB = bB->m_sweep.c + rB;
    const pB: Vec2 = Vec2.AddVV(bB.m_sweep.c, rB, Vec2.s_t1) // pB uses s_t1
    // Vec2 d = pB - pA;
