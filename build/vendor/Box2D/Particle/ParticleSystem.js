...
import { Abs, Clamp, InvSqrt, Max, Min, Rot, Sqrt, Transform, Vec2 } from '../Common/Math';
import { barrierCollisionTime, invalidParticleIndex, linearSlop, MakeArray, maxFloat, maxParticleForce, maxParticlePressure, maxTriadDistanceSquared, Maybe, minParticleSystemBufferCapacity, minParticleWeight, particleStride } from '../Common/Settings';
import { TimeStep } from '../Dynamics/TimeStep';
import { QueryCallback } from '../Dynamics/WorldCallbacks';
import { ParticleDef, ParticleFlag, ParticleHandle } from './Particle';
import { ParticleGroup, ParticleGroupDef, ParticleGroupFlag } from './ParticleGroup';
...
export class ParticleBodyContact {
    constructor() {
        ...
        this.normal = new Vec2(); // The normalized direction from the particle to the body.
...
export class ParticlePair {
    constructor() {
        ...
        this.indexB = 0;
        this.flags = 0; // The logical sum of the particle flags. See the ParticleFlag enum.
...
export class ParticleTriad {
    constructor() {
        ...
        this.flags = 0; // The logical sum of the particle flags. See the ParticleFlag enum.
        this.strength = 0.0; // The strength of cohesion among the particles.
        this.pa = new Vec2(0.0, 0.0); // Values used for calculation.
        this.pb = new Vec2(0.0, 0.0);
        this.pc = new Vec2(0.0, 0.0);
        this.ka = 0.0;
        this.kb = 0.0;
        this.kc = 0.0;
        this.s = 0.0;
...
export class ParticleSystem {
    constructor(def, world) {
         ...
         */
        this.m_handleIndexBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_flagsBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_positionBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_velocityBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_forceBuffer = [];
        /**
         ...
         */
        this.m_depthBuffer = [];
        this.m_colorBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_groupBuffer = [];
        this.m_userDataBuffer = new ParticleSystemUserOverridableBuffer();
        /**
         * Stuck particle detection parameters and record keeping
         */
        this.m_stuckThreshold = 0;
        this.m_lastBodyContactStepBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_bodyContactCountBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_consecutiveContactStepsBuffer = new ParticleSystemUserOverridableBuffer();
        this.m_stuckParticleBuffer = new GrowableBuffer(() => 0);
        this.m_proxyBuffer = new GrowableBuffer(() => new ParticleSystemProxy());
        this.m_contactBuffer = new GrowableBuffer(() => new ParticleContact());
        this.m_bodyContactBuffer = new GrowableBuffer(() => new ParticleBodyContact());
        this.m_pairBuffer = new GrowableBuffer(() => new ParticlePair());
         ...
         * seconds.
         */
        this.m_expirationTimeBuffer = new ParticleSystemUserOverridableBuffer();
        /**
         * List of particle indices sorted by expiration time.
         */
        this.m_indexByExpirationTimeBuffer = new ParticleSystemUserOverridableBuffer();
        /**
        ...
        this.m_def = new ParticleSystemDef();
        this.m_prev = null;
        this.m_next = null;
    ...
    static computeTag(x, y) {
        ...
        return (((((y + ParticleSystem.yOffset) >>> 0) << ParticleSystem.yShift) +
    ...
    static ParticleContactIsZombie(contact) {
        return ((contact.flags & ParticleFlag.zombieParticle) ===
    ...
    CreateParticle(def) {
        ...
        this.m_positionBuffer.data[index] = (this.m_positionBuffer.data[index] || new Vec2()).Copy(Maybe(def.position, Vec2.ZERO));
        this.m_velocityBuffer.data[index] = (this.m_velocityBuffer.data[index] || new Vec2()).Copy(Maybe(def.velocity, Vec2.ZERO));
        ...
        const color = new Color().Copy(Maybe(def.color, Color.ZERO));
        ...
        if (this.m_expirationTimeBuffer.data || finiteLifetime) {
            this.SetParticleLifetime(index, finiteLifetime
                ? lifetime
    ...
    DestroyParticlesInShape(shape, xf, callDestructionListener = false) {
        ...
        const callback = new ParticleSystemDestroyParticlesInShapeCallback(this, shape, xf, callDestructionListener);
        const aabb = s_aabb;
    ...
    JoinParticleGroups(groupA, groupB) {
        ...
        const filter = new ParticleSystemJoinParticleGroupsFilter(groupB.m_firstIndex);
        this.UpdateContacts(true);
    ...
    SplitParticleGroup(group) {
        ...
        const nodeBuffer = MakeArray(particleCount, (index) => new ParticleSystemParticleListNode());
    ...
    QueryAABB(callback, aabb) {
        ...
        const firstProxy = std_lower_bound(this.m_proxyBuffer.data, beginProxy, endProxy, ParticleSystem.computeTag(this.m_inverseDiameter * aabb.lowerBound.x, this.m_inverseDiameter * aabb.lowerBound.y), ParticleSystemProxy.CompareProxyTag);
        const lastProxy = std_upper_bound(this.m_proxyBuffer.data, firstProxy, endProxy, ParticleSystem.computeTag(this.m_inverseDiameter * aabb.upperBound.x, this.m_inverseDiameter * aabb.upperBound.y), ParticleSystemProxy.CompareTagProxy);
        if (!this.m_positionBuffer.data) {
            throw new Error();
        }
    ...
    QueryPointAABB(callback, point, slop = linearSlop) {
    ...
    CreateParticleForGroup(groupDef, xf, p) {
        ...
        Vec2.AddVV(Maybe(groupDef.linearVelocity, Vec2.ZERO), Vec2.CrossSV(Maybe(groupDef.angularVelocity, 0), Vec2.SubVV(particleDef.position, Maybe(groupDef.position, Vec2.ZERO), Vec2.s_t0), Vec2.s_t0), particleDef.velocity);
    ...
    CreateParticlesStrokeShapeForGroup(shape, groupDef, xf) {
        ...
        for (let childIndex = 0; childIndex < childCount; childIndex++) {
            ...
            while (positionOnEdge < edgeLength) {
                ...
                const p = Vec2.AddVMulSV(edge.m_vertex1, positionOnEdge / edgeLength, d, s_p);
        ...
        if (particleFlags & ParticleSystem.k_pairFlags) {
            for (let k = 0; k < this.m_contactBuffer.count; k++) {
                ...
                if (a >= firstIndex &&
                    ...
                    pair.strength = Min(groupA ? groupA.m_strength : 1, groupB ? groupB.m_strength : 1);
        ...
        if (particleFlags & ParticleSystem.k_triadFlags) {
            ...
            const callback = /*UpdateTriadsCallback*/ (a, b, c) => {
                ...
                if ((af | bf | cf) & ParticleSystem.k_triadFlags &&
                    ...
                    const maxDistanceSquared = maxTriadDistanceSquared * this.m_squaredDiameter;
                    ...
                    triad.strength = Min(Min(groupA ? groupA.m_strength : 1, groupB ? groupB.m_strength : 1), groupC ? groupC.m_strength : 1);
    ...
    ComputeDepth() {
        ...
        for (let group = this.m_groupList; group; group = group.GetNext()) {
            if (group.m_groupFlags & ParticleGroupFlag.particleGroupNeedsUpdateDepth) {
                groupsToUpdate[groupsToUpdateCount++] = group;
                this.SetGroupFlags(group, group.m_groupFlags & ~ParticleGroupFlag.particleGroupNeedsUpdateDepth);
                for (let i = group.m_firstIndex; i < group.m_lastIndex; i++) {
    ...
    GetInsideBoundsEnumerator(aabb) {
        const lowerTag = ParticleSystem.computeTag(this.m_inverseDiameter * aabb.lowerBound.x - 1, this.m_inverseDiameter * aabb.lowerBound.y - 1);
        const upperTag = ParticleSystem.computeTag(this.m_inverseDiameter * aabb.upperBound.x + 1, this.m_inverseDiameter * aabb.upperBound.y + 1);
        ...
        const firstProxy = std_lower_bound(this.m_proxyBuffer.data, beginProxy, endProxy, lowerTag, ParticleSystemProxy.CompareProxyTag);
        ///const Proxy* lastProxy = std::upper_bound(firstProxy, endProxy, upperTag);
        const lastProxy = std_upper_bound(this.m_proxyBuffer.data, beginProxy, endProxy, upperTag, ParticleSystemProxy.CompareTagProxy);
        // DEBUG: Assert(beginProxy <= firstProxy);
        // DEBUG: Assert(firstProxy <= lastProxy);
        // DEBUG: Assert(lastProxy <= endProxy);
        return new ParticleSystemInsideBoundsEnumerator(this, lowerTag, upperTag, firstProxy, lastProxy);
    }
    ...
    SortProxies(proxies) {
        ...
        std_sort(this.m_proxyBuffer.data, 0, this.m_proxyBuffer.count, ParticleSystemProxy.CompareProxyProxy);
    }
    FilterContacts(contacts) {
        ...
        const predicate = (contact) => {
            return ((contact.flags & ParticleFlag.particleContactFilterParticle) !== 0 &&
    ...
    UpdateBodyContacts() {
        ...
        const callback = new ParticleSystemUpdateBodyContactsCallback(this, this.GetFixtureContactFilter());
        this.m_world.QueryAABB(callback, aabb);
    ...
    Solve(step) {
        ...
        for (this.m_iterationIndex = 0; this.m_iterationIndex < step.particleIterations; this.m_iterationIndex++) {
            ...
            this.ComputeWeight();
            if (this.m_allGroupFlags & ParticleGroupFlag.particleGroupNeedsUpdateDepth) {
                this.ComputeDepth();
            }
    ...
    SolveGravity(step) {
        ...
        const gravity = Vec2.MulSV(step.dt * this.m_def.gravityScale, this.m_world.GetGravity(), s_gravity);
        for (let i = 0; i < this.m_count; i++) {
                        ...
                        if (cGroup && this.IsRigidGroup(cGroup)) {
                            ...
                            if (inertia > 0) {
                                ...
                                cGroup.m_angularVelocity +=
                                    Vec2.CrossVV(Vec2.SubVV(pc, cGroup.GetCenter(), Vec2.s_t0), f) / inertia;
                            }
                        }
                        else {
        ...
        for (let t = 0; t < this.m_def.staticPressureIterations; t++) {
            ...
            for (let i = 0; i < this.m_count; i++) {
                ...
                if (this.m_flagsBuffer.data[i] & ParticleFlag.staticPressureParticle) {
                    ...
                    const h = (wh + pressurePerWeight * (w - minParticleWeight)) /
                        (w + relaxation);
    ...
    SolveRigid(step) {
        ...
        for (let group = this.m_groupList; group; group = group.GetNext()) {
            if (group.m_groupFlags & ParticleGroupFlag.rigidParticleGroup) {
                ...
                const position = Vec2.AddVV(group.m_center, Vec2.SubVV(Vec2.MulSV(step.dt, group.m_linearVelocity, Vec2.s_t0), Rot.MulRV(rotation, group.m_center, Vec2.s_t1), Vec2.s_t0), s_position);
                const transform = s_transform;
    ...
    SolveTensile(step) {
        ...
        for (let k = 0; k < this.m_contactBuffer.count; k++) {
            ...
            if (contact.flags & ParticleFlag.tensileParticle) {
                ...
                const s = Vec2.SubVV(this.m_accumulation2Buffer[b], this.m_accumulation2Buffer[a], s_s);
                const fn = Min(pressureStrength * (h - 2) + normalStrength * Vec2.DotVV(s, n), maxVelocityVariation) * w;
                ///Vec2 f = fn * n;
    ...
    SolveZombie() {
        ...
        for (let i = 0; i < this.m_count; i++) {
            ...
            if (flags & ParticleFlag.zombieParticle) {
                ...
                if (flags & ParticleFlag.destructionListenerParticle &&
                    destructionListener) {
        ...
        for (let group = this.m_groupList; group; group = group.GetNext()) {
            ...
            if (firstIndex < lastIndex) {
                ...
                if (!(group.m_groupFlags & ParticleGroupFlag.particleGroupCanBeEmpty)) {
                    this.SetGroupFlags(group, group.m_groupFlags | ParticleGroupFlag.particleGroupWillBeDestroyed);
                }
            }
        }
    ...
    RemoveSpuriousBodyContacts() {
        ...
        // let discarded = 0;
        const ParticleBodyContactRemovePredicate = (contact) => {
            ...
            const pos = Vec2.AddVV(this.m_positionBuffer.data[contact.index], n, s_pos);
        ...
        this.m_bodyContactBuffer.count = std_remove_if(this.m_bodyContactBuffer.data, ParticleBodyContactRemovePredicate, this.m_bodyContactBuffer.count);
    }
    DetectStuckParticle(particle) {
        ...
        if (this.m_bodyContactCountBuffer.data[particle] === 2) {
            ...
            if (this.m_consecutiveContactStepsBuffer.data[particle] >
...
ParticleSystem.xTruncBits = 12;
ParticleSystem.yTruncBits = 12;
ParticleSystem.tagBits = 8 * 4; // 8u * sizeof(uint32);
ParticleSystem.yOffset = 1 << (ParticleSystem.yTruncBits - 1);
ParticleSystem.yShift = ParticleSystem.tagBits - ParticleSystem.yTruncBits;
ParticleSystem.xShift = ParticleSystem.tagBits -
    ParticleSystem.yTruncBits -
    ParticleSystem.xTruncBits;
ParticleSystem.xScale = 1 << ParticleSystem.xShift;
ParticleSystem.xOffset = ParticleSystem.xScale * (1 << (ParticleSystem.xTruncBits - 1));
ParticleSystem.yMask = ((1 << ParticleSystem.yTruncBits) - 1) << ParticleSystem.yShift;
ParticleSystem.xMask = ~ParticleSystem.yMask;
ParticleSystem.DestroyParticlesInShape_s_aabb = new AABB();
ParticleSystem.CreateParticleGroup_s_transform = new Transform();
ParticleSystem.ComputeCollisionEnergy_s_v = new Vec2();
ParticleSystem.QueryShapeAABB_s_aabb = new AABB();
ParticleSystem.QueryPointAABB_s_aabb = new AABB();
ParticleSystem.RayCast_s_aabb = new AABB();
ParticleSystem.RayCast_s_p = new Vec2();
ParticleSystem.RayCast_s_v = new Vec2();
ParticleSystem.RayCast_s_n = new Vec2();
ParticleSystem.RayCast_s_point = new Vec2();
/**
 * All particle types that require creating pairs
 */
ParticleSystem.k_pairFlags = ParticleFlag.springParticle;
/**
 * All particle types that require creating triads
 */
ParticleSystem.k_triadFlags = ParticleFlag.elasticParticle;
/**
 * All particle types that do not produce dynamic pressure
 */
ParticleSystem.k_noPressureFlags = ParticleFlag.powderParticle | ParticleFlag.tensileParticle;
/**
 * All particle types that apply extra damping force with bodies
 */
ParticleSystem.k_extraDampingFlags = ParticleFlag.staticPressureParticle;
ParticleSystem.k_barrierWallFlags = ParticleFlag.barrierParticle | ParticleFlag.wallParticle;
ParticleSystem.CreateParticlesStrokeShapeForGroup_s_edge = new EdgeShape();
ParticleSystem.CreateParticlesStrokeShapeForGroup_s_d = new Vec2();
ParticleSystem.CreateParticlesStrokeShapeForGroup_s_p = new Vec2();
ParticleSystem.CreateParticlesFillShapeForGroup_s_aabb = new AABB();
ParticleSystem.CreateParticlesFillShapeForGroup_s_p = new Vec2();
ParticleSystem.AddContact_s_d = new Vec2();
ParticleSystem.UpdateBodyContacts_s_aabb = new AABB();
ParticleSystem.Solve_s_subStep = new TimeStep();
ParticleSystem.SolveCollision_s_aabb = new AABB();
ParticleSystem.SolveGravity_s_gravity = new Vec2();
ParticleSystem.SolveBarrier_s_aabb = new AABB();
ParticleSystem.SolveBarrier_s_va = new Vec2();
ParticleSystem.SolveBarrier_s_vb = new Vec2();
ParticleSystem.SolveBarrier_s_pba = new Vec2();
ParticleSystem.SolveBarrier_s_vba = new Vec2();
ParticleSystem.SolveBarrier_s_vc = new Vec2();
ParticleSystem.SolveBarrier_s_pca = new Vec2();
ParticleSystem.SolveBarrier_s_vca = new Vec2();
ParticleSystem.SolveBarrier_s_qba = new Vec2();
ParticleSystem.SolveBarrier_s_qca = new Vec2();
ParticleSystem.SolveBarrier_s_dv = new Vec2();
ParticleSystem.SolveBarrier_s_f = new Vec2();
ParticleSystem.SolvePressure_s_f = new Vec2();
ParticleSystem.SolveDamping_s_v = new Vec2();
ParticleSystem.SolveDamping_s_f = new Vec2();
ParticleSystem.SolveRigidDamping_s_t0 = new Vec2();
ParticleSystem.SolveRigidDamping_s_t1 = new Vec2();
ParticleSystem.SolveRigidDamping_s_p = new Vec2();
ParticleSystem.SolveRigidDamping_s_v = new Vec2();
ParticleSystem.SolveExtraDamping_s_v = new Vec2();
ParticleSystem.SolveExtraDamping_s_f = new Vec2();
ParticleSystem.SolveRigid_s_position = new Vec2();
ParticleSystem.SolveRigid_s_rotation = new Rot();
ParticleSystem.SolveRigid_s_transform = new Transform();
ParticleSystem.SolveRigid_s_velocityTransform = new Transform();
ParticleSystem.SolveElastic_s_pa = new Vec2();
ParticleSystem.SolveElastic_s_pb = new Vec2();
ParticleSystem.SolveElastic_s_pc = new Vec2();
ParticleSystem.SolveElastic_s_r = new Rot();
ParticleSystem.SolveElastic_s_t0 = new Vec2();
ParticleSystem.SolveSpring_s_pa = new Vec2();
ParticleSystem.SolveSpring_s_pb = new Vec2();
ParticleSystem.SolveSpring_s_d = new Vec2();
ParticleSystem.SolveSpring_s_f = new Vec2();
ParticleSystem.SolveTensile_s_weightedNormal = new Vec2();
ParticleSystem.SolveTensile_s_s = new Vec2();
ParticleSystem.SolveTensile_s_f = new Vec2();
ParticleSystem.SolveViscous_s_v = new Vec2();
ParticleSystem.SolveViscous_s_f = new Vec2();
ParticleSystem.SolveRepulsive_s_f = new Vec2();
ParticleSystem.SolvePowder_s_f = new Vec2();
ParticleSystem.SolveSolid_s_f = new Vec2();
ParticleSystem.UpdatePairsAndTriads_s_dab = new Vec2();
ParticleSystem.UpdatePairsAndTriads_s_dbc = new Vec2();
ParticleSystem.UpdatePairsAndTriads_s_dca = new Vec2();
ParticleSystem.RemoveSpuriousBodyContacts_s_n = new Vec2();
ParticleSystem.RemoveSpuriousBodyContacts_s_pos = new Vec2();
ParticleSystem.RemoveSpuriousBodyContacts_s_normal = new Vec2();
export class ParticleSystemUserOverridableBuffer {
    constructor() {
        this.data = null;
...
export class ParticleSystemProxy {
    constructor() {
        this.index = invalidParticleIndex;
        this.tag = 0;
    }
...
export class ParticleSystemInsideBoundsEnumerator {
    ...
    GetNext() {
        while (this.m_first < this.m_last) {
            const xTag = (this.m_system.m_proxyBuffer.data[this.m_first].tag &
 ...
 * @constructor
 */
export class ParticleSystemFixedSetAllocator {
...
export class ParticleSystemFixtureParticle {
    constructor(fixture, particle) {
        this.second = invalidParticleIndex;
        this.first = fixture;
...
export class ParticleSystemParticlePair {
    constructor(particleA, particleB) {
        this.first = invalidParticleIndex;
        this.second = invalidParticleIndex;
...
export class ParticleSystemReactiveFilter extends ParticleSystemConnectionFilter {
    ...
    IsNecessary(index) {
        ...
        return ((this.m_flagsBuffer.data[index] & ParticleFlag.reactiveParticle) !== 0);
    }
}
export class ParticleSystemUpdateBodyContactsCallback extends FixtureParticleQueryCallback {
    ...
    ReportFixtureAndParticle(fixture, childIndex, a) {
        const s_n = ParticleSystemUpdateBodyContactsCallback.ReportFixtureAndParticle_s_n;
        const s_rp = ParticleSystemUpdateBodyContactsCallback.ReportFixtureAndParticle_s_rp;
        ...
        if (d < this.m_system.m_particleDiameter &&
            ...
            const invAm = this.m_system.m_flagsBuffer.data[a] & ParticleFlag.wallParticle
                ? 0
...
ParticleSystemUpdateBodyContactsCallback.ReportFixtureAndParticle_s_n = new Vec2();
ParticleSystemUpdateBodyContactsCallback.ReportFixtureAndParticle_s_rp = new Vec2();
export class ParticleSystemSolveCollisionCallback extends FixtureParticleQueryCallback {
    ...
    ReportFixtureAndParticle(fixture, childIndex, a) {
        const s_p1 = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_p1;
        const s_output = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_output;
        const s_input = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_input;
        const s_p = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_p;
        const s_v = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_v;
        const s_f = ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_f;
...
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_p1 = new Vec2();
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_output = new RayCastOutput();
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_input = new RayCastInput();
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_p = new Vec2();
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_v = new Vec2();
ParticleSystemSolveCollisionCallback.ReportFixtureAndParticle_s_f = new Vec2();
// #endif
