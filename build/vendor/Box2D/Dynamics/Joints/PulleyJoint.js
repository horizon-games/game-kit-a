...
import { Abs, Rot, Vec2 } from '../../Common/Math';
...
export class PulleyJoint extends Joint {
    ...
    InitVelocityConstraints(data) {
        ...
        const ruA = Vec2.CrossVV(this.m_rA, this.m_uA);
        const ruB = Vec2.CrossVV(this.m_rB, this.m_uB);
        ...
        if (data.step.warmStarting) {
            ...
            const PA = Vec2.MulSV(-this.m_impulse, this.m_uA, PulleyJoint.InitVelocityConstraints_s_PA);
            // Vec2 PB = (-m_ratio * m_impulse) * m_uB;
            const PB = Vec2.MulSV(-this.m_ratio * this.m_impulse, this.m_uB, PulleyJoint.InitVelocityConstraints_s_PB);
            // vA += m_invMassA * PA;
    ...
    SolveVelocityConstraints(data) {
        ...
        const vpA = Vec2.AddVCrossSV(vA, wA, this.m_rA, PulleyJoint.SolveVelocityConstraints_s_vpA);
        // Vec2 vpB = vB + Cross(wB, m_rB);
        const vpB = Vec2.AddVCrossSV(vB, wB, this.m_rB, PulleyJoint.SolveVelocityConstraints_s_vpB);
        const Cdot = -Vec2.DotVV(this.m_uA, vpA) - this.m_ratio * Vec2.DotVV(this.m_uB, vpB);
        const impulse = -this.m_mass * Cdot;
        this.m_impulse += impulse;
        // Vec2 PA = -impulse * m_uA;
        const PA = Vec2.MulSV(-impulse, this.m_uA, PulleyJoint.SolveVelocityConstraints_s_PA);
        // Vec2 PB = -m_ratio * impulse * m_uB;
        const PB = Vec2.MulSV(-this.m_ratio * impulse, this.m_uB, PulleyJoint.SolveVelocityConstraints_s_PB);
        // vA += m_invMassA * PA;
        vA.SelfMulAdd(this.m_invMassA, PA);
        wA += this.m_invIA * Vec2.CrossVV(this.m_rA, PA);
        // vB += m_invMassB * PB;
    ...
    SolvePositionConstraints(data) {
        ...
        // Compute effective mass.
        const ruA = Vec2.CrossVV(rA, uA);
        const ruB = Vec2.CrossVV(rB, uB);
        const mA = this.m_invMassA + this.m_invIA * ruA * ruA;
        const mB = this.m_invMassB + this.m_invIB * ruB * ruB;
        ...
        const linearError = Abs(C);
        const impulse = -mass * C;
        // Vec2 PA = -impulse * uA;
        const PA = Vec2.MulSV(-impulse, uA, PulleyJoint.SolvePositionConstraints_s_PA);
        // Vec2 PB = -m_ratio * impulse * uB;
        const PB = Vec2.MulSV(-this.m_ratio * impulse, uB, PulleyJoint.SolvePositionConstraints_s_PB);
        // cA += m_invMassA * PA;
    ...
    GetCurrentLengthA() {
        ...
        // Vec2 d = p - s;
        // return d.Length();
        const p = this.m_bodyA.GetWorldPoint(this.m_localAnchorA, PulleyJoint.GetCurrentLengthA_s_p);
    ...
    GetCurrentLengthB() {
        ...
        // Vec2 d = p - s;
        // return d.Length();
        const p = this.m_bodyB.GetWorldPoint(this.m_localAnchorB, PulleyJoint.GetCurrentLengthB_s_p);
...
PulleyJoint.InitVelocityConstraints_s_PA = new Vec2();
PulleyJoint.InitVelocityConstraints_s_PB = new Vec2();
PulleyJoint.SolveVelocityConstraints_s_vpA = new Vec2();
PulleyJoint.SolveVelocityConstraints_s_vpB = new Vec2();
PulleyJoint.SolveVelocityConstraints_s_PA = new Vec2();
PulleyJoint.SolveVelocityConstraints_s_PB = new Vec2();
PulleyJoint.SolvePositionConstraints_s_PA = new Vec2();
PulleyJoint.SolvePositionConstraints_s_PB = new Vec2();
PulleyJoint.GetCurrentLengthA_s_p = new Vec2();
PulleyJoint.GetCurrentLengthB_s_p = new Vec2();
//# sourceMappingURL=PulleyJoint.js.map