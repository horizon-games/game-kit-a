...
import { angularSleepTolerance, linearSleepTolerance, maxFloat, maxRotation, maxRotationSquared, maxTranslation, maxTranslationSquared, timeToSleep } from '../Common/Settings';
import { Timer } from '../Common/Timer';
import { BodyType } from './Body';
import { ContactSolver, ContactSolverDef } from './Contacts/ContactSolver';
import { Position, SolverData, Velocity } from './TimeStep';
import { ContactImpulse } from './WorldCallbacks';
/*
...
export class Island {
    constructor() {
        ...
        this.m_positions = Position.MakeArray(1024); // TODO: Settings
        this.m_velocities = Velocity.MakeArray(1024); // TODO: Settings
        this.m_bodyCount = 0;
        this.m_jointCount = 0;
    ...
    Initialize(bodyCapacity, contactCapacity, jointCapacity, allocator, listener) {
        ...
        if (this.m_positions.length < bodyCapacity) {
            const new_length = Max(this.m_positions.length * 2, bodyCapacity);
            while (this.m_positions.length < new_length) {
                this.m_positions[this.m_positions.length] = new Position();
            }
        }
        // TODO:
        if (this.m_velocities.length < bodyCapacity) {
            const new_length = Max(this.m_velocities.length * 2, bodyCapacity);
            while (this.m_velocities.length < new_length) {
                this.m_velocities[this.m_velocities.length] = new Velocity();
            }
        }
    }
    Clear() {
    ...
    AddContact(contact) {
        // DEBUG: Assert(this.m_contactCount < this.m_contactCapacity);
        this.m_contacts[this.m_contactCount++] = contact;
    }
    AddJoint(joint) {
        // DEBUG: Assert(this.m_jointCount < this.m_jointCapacity);
    ...
    Solve(profile, step, gravity, allowSleep) {
        ...
        for (let i = 0; i < this.m_bodyCount; ++i) {
            ...
            const translation = Vec2.MulSV(h, v, Island.s_translation);
            if (Vec2.DotVV(translation, translation) > maxTranslationSquared) {
                const ratio = maxTranslation / translation.Length();
                v.SelfMul(ratio);
            }
            const rotation = h * w;
            if (rotation * rotation > maxRotationSquared) {
                const ratio = maxRotation / Abs(rotation);
                w *= ratio;
            }
            // Integrate
            c.x += h * v.x;
        ...
        if (allowSleep) {
            ...
            for (let i = 0; i < this.m_bodyCount; ++i) {
                ...
                if (!b.m_autoSleepFlag ||
                    ...
                    Vec2.DotVV(b.m_linearVelocity, b.m_linearVelocity) > linTolSqr) {
                    b.m_sleepTime = 0;
                    minSleepTime = 0;
                }
                else {
...
Island.s_timer = new Timer();
Island.s_solverData = new SolverData();
Island.s_contactSolverDef = new ContactSolverDef();
Island.s_contactSolver = new ContactSolver();
Island.s_translation = new Vec2();
Island.s_impulse = new ContactImpulse();
