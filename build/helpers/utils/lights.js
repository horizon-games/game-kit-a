...
export function addPrettyLights(scene, bgColor) {
    const ambientLight = new HemisphereLight(__defaultSkyColor, __defaultGroundColor);
    ...
    const distance = 0.5;
    const angle = 0.6;
    sunLight.position.set(Math.cos(angle) * distance, Math.sin(angle) * distance, 0);
